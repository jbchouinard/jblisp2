...
pub fn jbuiltin_cons(args: JValRef, _env: JEnvRef, _state: &mut JState) -> JResult {
    let [x, y] = get_n_args(args)?;
    Ok(JVal::cons(Rc::clone(&x), Rc::clone(&y)))
}
...
pub fn jbuiltin_cdr(args: JValRef, _env: JEnvRef, _state: &mut JState) -> JResult {
    ...
    match &*list {
        ...
        _ => Err(JError::new("TypeError", "expected pair")),
    }
}
...
pub fn jbuiltin_is_list(args: JValRef, _env: JEnvRef, _state: &mut JState) -> JResult {
    let [val] = get_n_args(args)?;
    Ok(JVal::Bool(match &*val {
